...
/// the appliance between an agent and the world (see tasks/PHASE1-core.md and
...
pub fn kernel_nested(golden: &Path) -> Result<(), String> {
    ...
    if !src.is_dir() {
        ...
        kernel_canonical(&cella_libs::machine::kernel_path("canonical"))?;
    }
...
fn build_static_binaries() -> Result<(Vec<PathBuf>, PathBuf), String> {
    let root = repo_root();
    ...
    let probe = out.join("cella-probe");
    if !probe.is_file() {
        return Err(format!("{} did not build", probe.display()));
    }
    Ok((bins, probe))
}

/// The persona binaries an in-guest cella world carries (the probe
/// rides only the inception image).
const PERSONAS: &[&str] = &[
    "cella",
    "cella-machine",
    "cella-vmm",
...
fn rootfs_nested_family(
    ...
    let inner_kernel = cella_libs::machine::kernel_path("canonical");
    let inner_rootfs = cella_libs::machine::rootfs_path("canonical");
    ...
    let (bins, probe) = build_static_binaries()?;
    ...
    let inner_kernel_manifest = cella_libs::golden::manifest_path(&inner_kernel);
    let inner_rootfs_manifest = cella_libs::golden::manifest_path(&inner_rootfs);
    ...
    for b in bins {
        ...
        copies.push((b, nroot.join("bin").join(name), 0o755));
    }
    if with_jail {
