...
pub use cella_libs::sidecar::{QueueState, TransportState};
...
impl MmioTransport {
    ...
    pub fn restore_state(
        &mut self,
        st: &TransportState,
        open_ops: &[cella_libs::ledger::OpenOperation],
    ) {
        ...
        let outgoing: Vec<cella_libs::ledger::OpenOperation> =
            open_ops.iter().filter(|o| !o.incoming).cloned().collect();
        let incoming: Vec<cella_libs::ledger::OpenOperation> =
            open_ops.iter().filter(|o| o.incoming).cloned().collect();
    ...
    pub fn drain_ledger_events(&mut self) -> Vec<cella_libs::proto::Event> {
    ...
    pub fn restore_ingress(
        ...
        open: &[cella_libs::ledger::OpenOperation],
    ) {
    ...
    pub fn apply_decisions(
        &mut self,
        decisions: &std::collections::HashMap<Vec<u8>, cella_libs::proto::Decision>,
        mem: &GuestMemoryMmap,
    ) {
