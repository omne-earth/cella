...
"$BIN" \
    ...
    --tap "$TAP" \
    --mem-mb 128 \
    --cmdline "console=ttyS0 reboot=k panic=1 pci=off tsc=unstable clocksource=kvm-clock root=/dev/vda rw virtio_mmio.device=4K@0xd0000000:5 virtio_mmio.device=4K@0xd0001000:6 ip=${GUEST_IP}::${HOST_IP}:255.255.255.0::eth0:off" \
