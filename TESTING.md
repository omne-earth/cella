...
- **No KVM needed** (`make test`): `cargo test --lib`, `cargo test --tests`,
  `make jail`, `make seccomp`. These run in this repo's own dev sandbox,
...
- **Needs real KVM + a kernel/rootfs** (`make boot` / `make thaw` /
  `make net`): one target per feature that only exists once a guest can
...
2. Add a `<feature>: build ## ...` target to the `Makefile` that just
   calls the script.
3. Add it to `test-all`'s prerequisite list if it's KVM-dependent, or to
   `test`'s if it isn't.
