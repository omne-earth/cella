...
| `make smoke-udp` | **yes** | no datagram leaves undecided, proven from within the guest: a closed machine's UDP dies at the tap with no freeze, an open machine's UDP parks (proto and port in show) and the park is the freeze, a refusal delivers nothing to the host listener, and the guest's own ICMP lapses the same way, timing out in its own frame | `scripts/test/udp.sh` |
